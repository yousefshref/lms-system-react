...
const SchoolWebsite = () => {
  ...
  useEffect(() => {
    if (website?.details !== null || website?.details != []) setWebsiteHolder(website?.details || [])
  }, [website])
  ...
  const updateWebsite = () => {
    ...
    const images = websiteHolder?.filter(item => item?.check === 'image' && typeof item?.value === 'object')
  ...
  return (
    <div className='w-full max-w-7xl mx-auto'>
      {
        loading || loadingUpdate ? <LoadingScreen /> : (
          <div className='flex flex-col gap-8 p-5'>
            {
              ...
              <div className='p-3 flex flex-col justify-center rounded-xl bg-white shadow-md relative'>
                <span onClick={() => apiContext?.navigate(`/school/${profile?.school?.user_details?.username}/${profile?.school?.user_details?.id}/profile/`)} className='absolute left-3 cursor-pointer my-auto'>
                  <BiArrowBack />
                </span>
              </div>
              {/* elements */}
              <div className='flex gap-5 p-3 rounded-xl overflow-x-scroll bg-white shadow-md'>
                {
                  elements?.map(element => (
                    <WebsiteElement key={element?.id} type={element.check} element={element} />
                  ))
                }
              </div>

              <div>
                <button className='btn-primary' onClick={updateWebsite}>تحديث</button>
              </div>
              </>  
              )
            } 
            ...
            <div ref={drop} className='website p-3 rounded-xl bg-white shadow-md'>
              {
                ...
                websiteHolder?.map(element => (
                  <AddedElement is_admin={is_admin} setWebsiteHolder={setWebsiteHolder} websiteHolder={websiteHolder} key={element?.index} element={element} />
                ))
